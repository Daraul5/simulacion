...
class VistaProductosMedios(ft.Container):
    ...
    def procesar_datos(self, e):
        ...
        try:
            ...
            if len(val_s1) % 2 != 0:
                raise ValueError(
                    f"Semilla 1 tiene {len(val_s1)} dígitos. Debe ser par."
                )
            ...
            if len(val_s2) % 2 != 0:
                raise ValueError(
                    f"Semilla 2 tiene {len(val_s2)} dígitos. Debe ser par."
                )

            # Validación de longitud igual (opcional, dependiendo de tu core)
            if len(val_s1) != len(val_s2):
                raise ValueError(
                    "Ambas semillas deben tener la misma cantidad de dígitos."
                )
            ...
            if not val_n.isdigit() or int(val_n) <= 0:
                raise ValueError(
                    "Las iteraciones deben ser un número entero mayor a 0."
                )
